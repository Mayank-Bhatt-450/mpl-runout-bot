...
def lengths(im):#172 error
    ...
    if b !=[] and j!=[]:
        if b[0]<j[0]:
            pos='left'
            width=(j[len(j)-1]-j[0])+(j[len(j)-1]-j[endtoend(j)])
    ...
    for i in range(500):
        if tptr>14:
            print('--',j[0],ty+i,j[0]-ty+i)
            break
        else:
            pt=im.getpixel((ty+i,441))
            if (tell_color_dis(pt,(228,190,119),153.13392831113555)or inthis(color[0],color[1],pt )):
                tptr+=1
            else:
                tptr=0
...
while True:
    ...
    if (((check_color(k,red)or check_color(k,blue))and(check_dis(k)!=True))or check_color(blue_pts,blue)):
        ...
        if direction!=''and flag2!=1:
            ...
            d=input(direction+' '+str(f)+' '+str(lengths(im)))
            l=[direction,f,lengths(im),d]
            f = open('op.txt','a')
            f.write('\n' + str(l))
            f.close()
        flag2=1
