...
def lengths(im):
    ...
    for i in range(79):
        ...
        if tell_color_dis(im.getpixel((p+i,441)),[33,124,245],169.45205811674285):
            j=p+i
            break
    print(j,441)
    num=0
    ...
    k0=0
    while (441-k)>=0:
        print('''\n(2) ''',(j,441-k),tell_color_dis(im.getpixel((j,441-k)),[216,0,158],170.56227947264878
))
        if tell_color_dis(im.getpixel((j,441-k)),[216,0,158],150.56227947264878
):
            k0=1
        else:
            if k0==1:
...
while True:
    ...
    if (((check_color(k,red)or check_color(k,blue))and(check_dis(k)!=True))or check_color(blue_pts,blue)):
        ...
        if direction!='':
            print(direction,f,lengths(im))
        lengths(im)
        input('lengths over '+str(lengths(im)))
        '''
